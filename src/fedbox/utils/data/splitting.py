from typing import TypeVar, Optional, Callable, overload, cast
...
#         for count in counts:
#             if count > 0:
#                 part, rest = train_test_split(rest, train_size=int(count), stratify=stratify)
#             else:
#                 part = np.zeros((0, *array.shape[1:]), dtype=array.dtype)
#             result.append(part)
#         return result
...
@overload
def _shuffle_together(array1: NDArray1, array2: NDArray2, /) -> tuple[NDArray1, NDArray2]: ...


@overload
def _shuffle_together(
    array1: np.ndarray,
    array2: np.ndarray,
    array3: np.ndarray,
    /,
